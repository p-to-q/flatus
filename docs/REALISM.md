...
The three planned knobs landed in `crates/fart-synth/src/graph.rs`. Below are
the actual implementations + listen verdict, both as honestly as possible.

### What shipped

**#1 — aperiodic tremor.** Replaced the previous sine LFO (4–25 Hz fixed
frequency, depth × sin abs) with a low-pass-filtered random walk: sample a
new target every 30 ms from the per-event `Mulberry32`, 1-pole LPF the path
toward it with α = exp(−2π·3/sr) (≈ 0.99961 at 48 kHz, ≈ 3 Hz cutoff). Depth
still scales linearly with `params.tremor`. ~24 LOC. The mechanical sing-song
quality of v0.3 is gone; the wobble now drifts.

**#2 — bubble-burst transients.** Poisson-distributed Gaussian-windowed sine
bursts inserted between the grain renderer and the tremor stage. Density =
`16·wetness + 6·crackle` events/second, strength = `0.16 + 0.24·crackle`. The
burst centre frequency is **personality-coupled**: `1.2× – 2.4× centre_hz`,
so a biblical event (centre 110 Hz) pops at 130–270 Hz and a polite-cough
event (centre 220 Hz) pops at 265–530 Hz. Bigger cavities make bigger
bubbles make lower pops — this is the actual Helmholtz physics, and it keeps
the plausibility-test invariants happy (the test failed once when pops were
fixed at 400–800 Hz and pulled biblical's energy out of its claimed band;
the centre-coupling fix kept biblical at 0.96 → 1.4+ in-band/above-band
ratio). ~28 LOC.

**#4 — pink/brown noise mix.** New `BrownNoise` integrator (leaky random
walk, clamped to ±1, ×3.5 gain compensation) runs in parallel with the
existing `PinkNoise`. Per-grain source noise is now
`(1 − w)·pink + w·brown` with `w = 0.55·wetness`. Dry voices stay
pink-heavy; wet voices pick up ~55 % brown. ~16 LOC plus the small new
struct definition.

Total: ~68 LOC of synthesis change. Below the 90 LOC estimate; the
centre-coupled bursts avoided needing a separate personality dimension.

### Goldens — v0.3 → v0.4 SHA-256 diffs

All four canonical fixtures regenerated by `cargo run --release --bin
generate-goldens`. v0.3 archive at `fixtures/golden/v0.3/`,
`apps/web/samples/v0.3/` and `flatus.vercel.app/samples/v0.3/` for A/B.

| Personality | v0.3 SHA-256 | v0.4 SHA-256 |
|---|---|---|
| polite-cough | `3203a4a3…` | `57ea18db…` |
| default | `1fede9b2…` | `6cba6a51…` |
| biblical | `078f5338…` | `48e99c42…` |
| silent-but-deadly | `1ccbea96…` | `d1f2a0fb…` |

Full hashes live in `fixtures/golden/manifest.json` and
`fixtures/golden/v0.3/manifest.json`.

### Validation

- ✅ **Plausibility test** (`tests/plausibility.rs`): all four personalities
  concentrate energy in their claimed bands. Required the bubble-burst
  centre-coupling fix to pass (see above).
- ✅ **Output-cap test**: peak amplitude still under −6 dBFS speaker / −18
  dBFS headphones across all four personalities.
- ✅ **Render determinism test**: same `(personality, seed, pressure)` →
  byte-identical output across runs.
- ⏭ **Listen panel**: deferred — user explicitly waived this as a release
  gate ("不要人工复核了"). The v0.3 archive at `samples/v0.3/` is the
  reproducible A/B if anyone wants to do it later.

### What's out (kept from the original plan)

The three deferred knobs — **#3 formants**, **#5 decay tail**, **#6
saturation gating** — are not in v0.4. The "Out of scope" section above
still stands.

### Side effects landed in the same PR

- v0.3 goldens archived to `fixtures/golden/v0.3/` and to
  `apps/web/samples/v0.3/` (web-hosted for direct comparison).
- v0.4 goldens hosted at `apps/web/samples/v0.4/`; specimen cards on
  flatus.vercel.app gained a `↓ .wav` download link per card pointing at
  the canonical golden.
- README's Listen section flipped from GitHub-raw URLs to Vercel-hosted
  URLs (faster CDN, doesn't depend on github.com for media delivery).
- Data visuals (`docs/screenshots/waveforms-all.png`,
  `spectrogram-biblical.png`) re-rendered from the new goldens.

---

*Originally authored as the v0.4 planning step. Results section appended
on implementation. The next realism pass (formants / decay tail / sat
gating) gets its own document when scheduled.*
